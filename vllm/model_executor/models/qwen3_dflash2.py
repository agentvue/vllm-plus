...
class DFlash2Qwen3Model(DFlashQwen3Model):
    ...
    def __init__(
        ...
        *,
        ...
        trained_block_size = int(config.dflash_config["block_size"])
        requested_block_size = 1 + speculative_config.num_speculative_tokens
        if requested_block_size != trained_block_size:
            raise ValueError(
                "DFlash2 requires num_speculative_tokens="
                f"{trained_block_size - 1} for its trained block size; got "
