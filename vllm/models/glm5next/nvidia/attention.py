...
class Glm5NextIndexerCache(DeepseekV32IndexerCache):
    ...
    *content* (softmax-weighted sum vs keep-every-Nth) is computed by the
    ...
    needs ``block_size`` to be a multiple of the architecture's minimum pool
    page: 64 entries on SM120 FP8 and 32 entries elsewhere.
    """
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig):
        ...
        # SM120's FP8 paged-MQA kernel accepts only block_kv=64. Other CUDA
        # architectures also accept the 32-entry page.
        page_size = (
        ...
        assert (
            ...
            and storage_block_size % page_size == 0
        ), (
            "Glm5NextIndexerCache: kpool indexer requires cache block_size to "
            "be a multiple of index_kpool * page_size "
            f"({self._index_kpool * page_size}) so that DeepGEMM paged-MQA "
            f"pool pages ({page_size} entries) tile the "
