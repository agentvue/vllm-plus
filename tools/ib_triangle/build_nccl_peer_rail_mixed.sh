...
INSTALL_DIR="${2:-$HOME/nccl-matrix/nccl-cu13-2.30.7-peerrail-topo-sm86-sm120}"
ARCHIVE_PATH="${3:-$HOME/nccl-2.30.7-peerrail-topo-sm86-sm120.tar.gz}"
NCCL_NVCC_GENCODE="-gencode=arch=compute_86,code=sm_86 -gencode=arch=compute_120,code=sm_120 -gencode=arch=compute_120,code=compute_120"
...
echo "Building NCCL device code for native SM86 and SM120 plus compute_120 PTX."
env -u BUILDDIR -u CUDARTLIB -u NVCC_GENCODE -u ONLY_FUNCS \
...
if ! grep -Fq 'sm_86' <<<"$cubin_list"; then
  echo "Built NCCL library does not contain a native SM86 cubin." >&2
  exit 1
fi
echo "$cubin_list" | grep -F 'sm_86'
